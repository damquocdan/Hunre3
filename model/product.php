...
function listProduct($limit)
{
	...
	$sql = "SELECT *
	FROM product p, product_color pc, product_size ps WHERE p.id_pro = pc.id_pro AND p.id_pro = ps.id_pro
	AND color <> '' AND size <> '' AND p.status = 0
	GROUP BY p.id_pro, p.pro_name
