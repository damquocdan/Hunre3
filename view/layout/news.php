<section class="news">
	<div class="container">
		<div class="news-header">
			<h2 class="heading">Tin tức nhà ở</h2>
			<div class="news-read-all">
				<a href="#">Xem tất cả</a>
				<i class="fa fa-angle-right"></i>
			</div>
		</div>
	</div>
	<div class="news-list">
		<div class="news-item">
			<div class="news-image">
				<a href="#" class="news-link">
					<img src="././public/images/new1.png" alt="" />
				</a>
			</div>
			<div class="news-detail">
				<h3 class="news-title">
					<a href="#">
						Top 20 phòng ở khu vực được yêu thích nhất
					</a>
				</h3>
			</div>
		</div>
		<div class="news-item">
			<div class="news-image">
				<a href="#" class="news-link">
					<img src="././public/images/new7.png" alt="" />
				</a>
			</div>
			<div class="news-detail">
				<h3 class="news-title">
					<a href="#">
						Sinh viên các trường đại học nên thuê phòng trọ như nào
					</a>
				</h3>
			</div>
		</div>
		<div class="news-item">
			<div class="news-image">
				<a href="#" class="news-link">
					<img src="././public/images/new3.png" alt="" />
				</a>
			</div>
			<div class="news-detail">
				<h3 class="news-title">
					<a href="#">
						Những khu vực có giá trọ tiền đắt giá nhất
					</a>
				</h3>
			</div>
		</div>
		<div class="news-item">
			<div class="news-image">
				<a href="#" class="news-link">
					<img src="././public/images/new5.png" alt="" />
				</a>
			</div>
			<div class="news-detail">
				<h3 class="news-title">
					<a href="#">
						Các bạn nữ nên ở khu vực nào để an toàn
					</a>
				</h3>
			</div>
		</div>
		<div class="news-item">
			<div class="news-image">
				<a href="#" class="news-link">
					<img src="././public/images/new4.png" alt="" />
				</a>
			</div>
			<div class="news-detail">
				<h3 class="news-title">
					<a href="#">
						Top 10 khu trọ có địa lý vị trí tuyệt vời nhất
					</a>
				</h3>
			</div>
		</div>
		<div class="news-item">
			<div class="news-image">
				<a href="#" class="news-link">
					<img src="././public/images/new8.png" alt="" />
				</a>
			</div>
			<div class="news-detail">
				<h3 class="news-title">
					<a href="#">

						10 cách phòng trống nạn cháy các khu trọ

					</a>
				</h3>
			</div>
		</div>
		<div class="news-item">
			<div class="news-image">
				<a href="#" class="news-link">
					<img src="././public/images/new2.png" alt="" />
				</a>
			</div>
			<div class="news-detail">
				<h3 class="news-title">
					<a href="#">

						Các mặt kinh doanh đẹp nhất trên các con phố Hà Nội

					</a>
				</h3>
			</div>
		</div>
	</div>
</section>